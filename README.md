...
| **resolution_state** | VARCHAR | No | CHECK (`RESOLVED`, `AMBIGUOUS`, `PARTIAL`, `UNRESOLVED`) |
| **ai_summary** | TEXT | Yes | |
...
- **`resolution_state`**: The core state engine value of the application. Restricted by a `CHECK` constraint to exactly four mutually exclusive states: `RESOLVED`, `AMBIGUOUS`, `PARTIAL`, or `UNRESOLVED`.
